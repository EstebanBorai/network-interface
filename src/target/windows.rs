...
use winapi::{
    shared::{
        ifdef::IF_LUID,
        netioapi::{ConvertInterfaceLuidToIndex, ConvertLengthToIpv4Mask},
        ntdef::ULONG,
...
use crate::{Error, NetworkInterface, NetworkInterfaceConfig, Result, utils};
...
impl NetworkInterfaceConfig for NetworkInterface {
    fn show() -> Result<Vec<NetworkInterface>> {
        ...
        let mut adapter_address = utils::ffialloc::FFIAlloc::<AdapterAddress>::new(size_pointer as usize);
        let mut get_adapter_addresses_result = ERROR_BUFFER_OVERFLOW;
        let mut network_interfaces: Vec<NetworkInterface> = Vec::new();

        for _ in 0..MAX_TRIES {
            if !adapter_address.realloc(size_pointer as usize) {
            ...
            if get_adapter_addresses_result != ERROR_BUFFER_OVERFLOW { break; }
        }

        if get_adapter_addresses_result != GET_ADAPTERS_ADDRESSES_SUCCESS_RESULT {
            use std::io::Error as stde;
            return Err(Error::GetIfAddrsError(stde::from_raw_os_error(get_adapter_addresses_result as _).to_string(),
                                              get_adapter_addresses_result as _));
        }

        let mut adapter_address = adapter_address.as_ptr();
...
mod tests {
    ...
    use crate::{Addr, NetworkInterface, NetworkInterfaceConfig};
